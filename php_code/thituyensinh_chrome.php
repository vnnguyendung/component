...
$crawler = $client->request('GET', 'http://localhost/thituyensinh/r.htm');
// d($crawler);
...
///// phan than
 ...
 $demso = $xuly1->count();
 	...
 	echo '<div  class="copy-content"><div id="id-thong-tin-chung" >';
 ...
 echo '</div></div>
      <button id="copy-thong-tin-chung" class="copy-btn footer-1">Copy Thong tin 1</button>';
...
/* $crawler->filter('td.xpt div.khaithac_body > div table.x1y3')->eq(0)->each(function ($node){
 ...
 table_tuyensinh();
 	...
 	echo '<div   class="copy-content"> <div id="id-thong-tin-daydu">';
 $crawler->filter('td.xpt div.khaithac_body > div table.x1y3')->eq(1)->each(function ($node){
 		  		echo $node->html();
 });
echo '</div></div>
      <button id="copy-thong-tin-daydu" class="copy-btn footer-3">Copy thong tin 3</button>';
...
$objPHPExcel = PHPExcel_IOFactory::load(dirname(__FILE__) . "/../../thituyensinh/r.xlsx");
foreach ($objPHPExcel->getWorksheetIterator() as $key => $worksheet) {
  ...
  echo 'TRINH DO DAI HOC <div class="copy-content">';

    echo '<table id="table-ts-daihoc">';
    ...
    echo '</table>';
    echo '</div>
      <button id="copy-thong-tin-daihoc" class="copy-btn footer-dh">Copy thong tin DHAI HOC</button>';



 echo 'TRINH DO CAO DANG <div class="copy-content">';

    echo '<table id="table-ts-caodang">';
    // $row = 3 de bo 2 row dau
	// dem so luong cao dang
	//$count_cd = 0;
	// co col hay khong thi van show tr, lam vo khung thong tin

    for ($row = 3; $row <= $highestRow; ++ $row) {
    	// tr khi co trinh do cao dang, 
        echo '<tr>';
        // col = 2 de bo 2 col dau.
        for ($col = 0; $col < $highestColumnIndex; ++ $col) {
        		...
        		// loc chi trinh do cao dang, neu la trinh do dai hoc thi bo qua
    ...
    echo '</table>';
    echo '</div>
      <button id="copy-thong-tin-caodang" class="copy-btn footer-cd">Copy thong tin CAO DANG</button>';
